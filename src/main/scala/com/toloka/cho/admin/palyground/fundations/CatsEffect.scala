package com.toloka.cho.admin.palyground.fundations
...
//  }
  ...
  val delayedPrint = IO(println("Starting process")) *> IO.sleep(1.second) *> IO(println(Random.nextInt(100)))
  val manyPrints = for {
  ...
  val ignoredCancellation = for {
    fib <- IO.uncancelable(_ => delayedPrint.onCancel(IO(println("Receiving Cancel Signal but I'm Uncancelable !")))).start
    _ <- IO.sleep(500.millis) *> IO(println("Cancelling fiber")) *> fib.cancel
    _ <- fib.join
  } yield ()
